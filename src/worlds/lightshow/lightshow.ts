...
import { xrRender, xrHandState, getXRRig } from '../../vr';
import { DrumKit, type Kind } from './drums';
import { RhythmGame, type RhythmAudio } from './rhythm/game';
import { makeBloomComposer } from '../../utils/composer';
import { deadzoneJoystick } from '../../utils/math';
import type { World, WorldFrame } from '../../utils/world';
...
export class LightShowWorld implements World {
  ...
  private manBeat = 0;
  // rythme des effets : 'auto' = analyse de la musique, 'manual' = geste (poing)
  private rhythmMode: 'auto' | 'manual' = 'auto';
  private lastState: GestureState | null = null;

  // batterie VR : visible uniquement en session immersive
  private drums: DrumKit;
  // mode jeu rythme (Guitar Hero batterie), activable au clic du stick gauche
  private rhythm: RhythmGame;
  private prevLsClick = false;
  private drumHits: Kind[] = [];

  constructor(
    private renderer: THREE.WebGLRenderer,
    private audio: RhythmAudio,
  ) {
    this.drums = new DrumKit(() => audio.output());
    this.drums.onHit = (kind) => this.drumHits.push(kind);
    this.rhythm = new RhythmGame(audio);
  ...
  /** Quitte le monde musique : coupe le jeu rythme (et son groove) en cours. */
  deactivate(): void {
    if (this.rhythm.active) this.rhythm.stop();
  }
  ...
  /** Bascule le rythme des effets auto ↔ manuel ; renvoie le mode courant. */
  toggleRhythmMode(): 'auto' | 'manual' {
    this.rhythmMode = this.rhythmMode === 'auto' ? 'manual' : 'auto';
  ...
  applyGestures(g: GestureState): void {
    ...
    this.camManual = !!sec;
    const j = sec ? deadzoneJoystick(sec.cursor) : { x: 0, y: 0 };
  ...
  private manualFrame(g: GestureState, dt: number): AudioFrame {
  ...
  /**
   * En VR : baguettes dans les mains, mode jeu rythme géré au clic du stick
   * gauche, et frappes (collectées via `DrumKit.onHit`) notées + injectées dans
   * le mix — les effets du light show réagissent aux coups comme à la musique.
   * Hors VR : on détache la batterie et on coupe le jeu en cours.
   */
  private mixDrums(frame: AudioFrame, dt: number): AudioFrame {
    ...
    const rig = getXRRig(this.scene, this.camera);
    this.drums.attach(rig);
    this.rhythm.attach(rig);
    xrHandState.hideMarkers = true; // les baguettes remplacent les sphères

    // clic du stick gauche : démarre / arrête le mode jeu rythme
    const lsClick = xrHandState.gamepads[1]?.buttons[3]?.pressed === true;
    if (lsClick && !this.prevLsClick) this.rhythm.toggle();
    this.prevLsClick = lsClick;

    // frappes de la frame → notation du jeu, puis fusion dans le mix
    this.drumHits.length = 0;
    const d = this.drums.update(dt);
    this.rhythm.followKit(this.drums.offset); // l'autoroute suit le kit déplacé
    this.rhythm.update(dt, this.drumHits);
    this.flashEnv = Math.max(this.flashEnv, this.rhythm.popFlash());

    return {
      bass: Math.min(1, frame.bass + d.bass),
      mid: Math.min(1, frame.mid + d.mid),
      high: Math.min(1, frame.high + d.high),
      level: Math.min(1, frame.level + d.level * 0.6),
      beat: Math.max(frame.beat, d.beat),
      playing: frame.playing || d.beat > 0.02,
    };
  }
  ...
  render(): void {
    ...
    // rythme des effets : analyse de la musique (auto) ou geste (manuel)
    let frame =
      this.rhythmMode === 'manual' && this.lastState
        ? this.manualFrame(this.lastState, dt)
        : this.audio.frame();

    // batterie VR + mode jeu rythme : frappes injectées dans le mix
    frame = this.mixDrums(frame, dt);
    ...
    this.camera.position.setFromSphericalCoords(r, this.camPhi, this.camTheta).add(target);
    this.camera.lookAt(target);

    // en VR : poste de batteur fixe au sol, en recul face à la scène
    // (orientation yaw seule — pencher le rig donnerait le mal de mer ;
    //  bouton A/X de la manette pour alterner recul / rapproché)
    if (this.renderer.xr.isPresenting) {
      this.camera.position.set(0, this.wave.position.y, this.drums.standDistance);
