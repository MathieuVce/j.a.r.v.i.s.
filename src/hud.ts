...
const WORLD_ORDER: WorldKind[] = ['map', 'lightshow', 'universe'];
...
const HELP: Record<HudContext, string> = {
  universe:
    '✋ MAIN OUVERTE EXCENTRÉE = TOURNER (CENTRE = STOP) · 🤏🤏 ÉCARTER/RAPPROCHER = ZOOM · 🤏 PINCER = ATTRAPER',
  globe:
    '✋ MAIN OUVERTE EXCENTRÉE = TOURNER (CENTRE = STOP) · 🤏 PINCER UN POINT = PLONGER · 🤏🤏 = ZOOM',
  city:
    '✋ HAUT/BAS = AVANCER/RECULER · ✋ GAUCHE/DROITE = PIVOTER · 🤏 TIRER = DÉPLACER · 🤏🤏 = ALTITUDE · ✊ = RETOUR GLOBE',
  lightshow:
...
export class HUD {
  ...
  constructor() {
    this.worldButton.addEventListener('click', () => {
      const next =
        WORLD_ORDER[(WORLD_ORDER.indexOf(this.world) + 1) % WORLD_ORDER.length];
      this.setWorld(next);
    ...
    document.querySelectorAll<HTMLButtonElement>('.ls-track').forEach((btn) => {
      btn.addEventListener('click', () =>
        this.onLsTrack?.(btn.dataset.url!, btn.dataset.name!),
      );
    });
  ...
  setContext(ctx: HudContext): void {
    ...
    this.context = ctx;
    this.help.textContent = HELP[ctx];
  ...
  update(g: GestureState, labelOverride?: string | null, charge = 0): void {
    this.modeLabel.textContent =
      labelOverride ?? LABELS[this.context][g.mode] ?? 'STANDBY';
